...
export default function ChangePasswordPage() {
  ...
  const handleRequestCode = async (e: React.FormEvent) => {
    ...
    if (!sendVia.email && !sendVia.whatsapp) {
      toast.error("Selecciona al menos un método de verificación");
      return;
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.error("No se pudo reenviar el código. Intenta de nuevo.", { id: toastId });
    } finally {
      setResending(false);
    }
  };
  ...
  return (
    <div className="bg-white-primary rounded-primary p-6 col-span-full max-w-lg flex flex-col gap-8">
      {/* Encabezado */}
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <Checkbox
                  ...
                  onCheckedChange={(v) => setSendVia((s) => ({ ...s, email: !!v }))}
                />
              ...
              {profile.phone.length > 0 && (
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setSendVia((s) => ({ ...s, whatsapp: !!v }))}
                  />
                  <MessageSquare className="size-4 text-muted-foreground" />
                  <span className="text-sm">
                    WhatsApp
                    <span className="text-muted-foreground ml-1">({profile.phone})</span>
                  </span>
                </label>
              )}
